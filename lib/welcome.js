...
// Cooldown per grup: cegah spam welcome kalau banyak join sekaligus
const welcomeCooldown = new Map();
const COOLDOWN_MS = 4000; // 4 detik per grup

module.exports.welcome = async (iswel, isleft, hydro, anu) => {
  try {
    // Cek cooldown per grup
    const lastSent = welcomeCooldown.get(anu.id) || 0;
    const now_cd = Date.now();
    if (now_cd - lastSent < COOLDOWN_MS) return;
    welcomeCooldown.set(anu.id, now_cd);
    ...
    for (let i = 0; i < participants.length; i++) {
      const num = participants[i];
      // Delay antar member supaya tidak request ke WA sekaligus (hindari rate limit)
    ...
    if (err?.data === 429 || err?.message?.includes('rate')) {
      console.log('[Welcome] Rate limit WA, skip.');
    } else if (err?.data === 403 || err?.message?.includes('forbidden')) {
      console.log('[Welcome] Forbidden (bot bukan anggota grup), skip.');
    } else {
