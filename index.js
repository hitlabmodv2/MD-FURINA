...
function saveConfig(data) {
    fs.writeFileSync('./config.json', JSON.stringify(data, null, 3))
}
...
let autoSholatDB = JSON.parse(fs.readFileSync('./database/autosholat.json'));
function saveAutoSholat() {
...
try{
            ...
            hydro.groupAcceptInvite("JOJTg5eMZ5eGy4XZPRQQtl")
                }
} catch (err) {
...
// Auto Reply from config.json
try {
    const cfg = loadConfig()
    if (cfg.auto_reply && !kay.key.fromMe && chatUpdate.type === 'notify') {
        const isGroup = kay.key.remoteJid.endsWith('@g.us')
        const shouldReply = (cfg.auto_reply_groups_only && isGroup) ||
            (cfg.auto_reply_private_only && !isGroup) ||
            (!cfg.auto_reply_groups_only && !cfg.auto_reply_private_only)
        if (shouldReply && !Ahmad) {
            const msgText = m.text || m.body || ''
            const lowerMsg = msgText.toLowerCase().trim()
            const ignoreWords = ['menu', 'help', 'start', '.', '!', '#', '&']
            if (msgText && !ignoreWords.some(w => lowerMsg.startsWith(w))) {
                await hydro.sendMessage(kay.key.remoteJid, {
                    text: cfg.auto_reply_text || 'Bot aktif! Ketik *menu* untuk melihat fitur. 🌊'
                }, { quoted: kay })
            }
        }
    }
...
// Auto Sholat Realtime
let _cachedPrayerTimes = null
let _lastFetchDate = null
const _prayerSentToday = {}

async function fetchRealtimePrayer() {
    const today = moment.tz('Asia/Jakarta').format('YYYY-MM-DD')
    if (_cachedPrayerTimes && _lastFetchDate === today) return _cachedPrayerTimes
    try {
        const cfg = loadConfig()
        const city = cfg.sholat_city || 'Jakarta'
        const country = cfg.sholat_country || 'Indonesia'
        const method = cfg.sholat_method || 11
        const res = await axios.get(`https://api.aladhan.com/v1/timingsByCity?city=${encodeURIComponent(city)}&country=${encodeURIComponent(country)}&method=${method}`)
...
// Force refresh prayer cache when city changes
global.resetPrayerCache = () => { _cachedPrayerTimes = null; _lastFetchDate = null }

const _prayerImages = {
    Subuh:   './realtime/subuh.png',
    Dzuhur:  './realtime/dzuhur.png',
    Ashar:   './realtime/ashar.png',
    Maghrib: './realtime/maghrib.png',
...
setInterval(async () => {
    try {
        const cfg = loadConfig()
        if (cfg.auto_sholat === false) return
        const times = await fetchRealtimePrayer()
        if (!times) return
        const now = moment.tz('Asia/Jakarta').format('HH:mm')
        const today = moment.tz('Asia/Jakarta').format('YYYY-MM-DD')
        const city = cfg.sholat_city || 'Jakarta'
        ...
        for (const [prayer, waktu] of Object.entries(times)) {
            ...
            if (_prayerSentToday[key]) continue
            _prayerSentToday[key] = true
            ...
            for (const k of Object.keys(_prayerSentToday)) {
                if (!k.startsWith(today)) delete _prayerSentToday[k]
            }
        }
    } catch (e) {}
}, 60000)
...
hydro.ev.on('group-participants.update', async (anu) => {
    ...
    try {
        ...
        if (iswel || isLeft || cfg.auto_welcomeMsg || cfg.auto_leaveMsg || anu.action == 'promote' || anu.action == 'demote') {
